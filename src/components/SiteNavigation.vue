    ...
    >
      ...
      <div class="flex gap-3 flex-1 justify-end">
        ...
        <i
          ...
          @click="addCityForTracking()"
          v-if="route.query.preview && !isCityAddedForTracking"
        ></i>
      </div>
...
const isCityAddedForTracking = ref(false);
const toggleModal = () => {
  modalActive.value = !modalActive.value;
};

const addCityForTracking = () => {
...
watch(route, async (newRoute, oldRoute) => {
  let presentCities = [];
  if (localStorage.getItem("savedCities")) {
    presentCities = JSON.parse(localStorage.getItem("savedCities"));
  }
  presentCities?.map((data) => {
    if (data?.city == route.params.city) {
      isCityAddedForTracking.value = true;
    }
  });
});
</script>
